...
const whereAmI = function(lat, lng){
  ...
  res => {
    if(res.status === 403) {
     throw new Error('Reloaded too many times!');
    }
    res.json()})
.then(data => {
...
throw new Error(`Failed to reload ${res.status}`)}
return res.json})
...
PART 1
1. Create a function 'createImage' which receives imgPath as an input. This function returns a promise which creates
 a new image (use document.createElement('img')) and sets the .src attribute to the provided image path. 
...
const loadNPause = async function() { try{
...
const loadAll = async function(imgArr) { try {
  const createimgs = await imgArr.map(async img => await createImage(img));
  console.log(createimgs());
  const imgEl = PromiseAll(createimgs);
  console.log(imgEl);
